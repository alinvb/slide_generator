                ...
                if not in_string:
                    if char == '{':
                        ...
                        if bracket_count == 0 and brace_count == 0:
                            # Found end of first JSON array
                            return json_str[:i+1]
        
        # Handle missing comma between objects - ENHANCED VERSION
        elif "Expecting ',' delimiter" in str(e):
            print(f"[JSON REPAIR] Fixing delimiter error at position {error_pos}...")
            
            # Character-level analysis around error position
            context_start = max(0, error_pos - 100)
            context_end = min(len(json_str), error_pos + 100)
            context = json_str[context_start:context_end]
            print(f"[JSON REPAIR] Context around error: {repr(context)}")
            
            # Method 1: Direct character insertion at error position
            if error_pos > 0 and error_pos < len(json_str):
                char_before = json_str[error_pos - 1]
                char_at_pos = json_str[error_pos] if error_pos < len(json_str) else ''
                
                print(f"[JSON REPAIR] Character before error: {repr(char_before)}, at error: {repr(char_at_pos)}")
                
                # Insert comma if we have closing brace/bracket followed by opening
                if char_before in '}]"' and char_at_pos in '{["':
                    print(f"[JSON REPAIR] Inserting comma at position {error_pos}")
                    json_str = json_str[:error_pos] + ',' + json_str[error_pos:]
                    return json_str
            
            # Method 2: Enhanced line-by-line analysis with better pattern detection
            ...
            for i, line in enumerate(lines):
                if chars_count + len(line) >= error_pos:
                    error_line = i
                    break
                chars_count += len(line) + 1
            
            print(f"[JSON REPAIR] Error detected around line {error_line}")
            
            # Analyze patterns around error line with extended context
            start_check = max(0, error_line - 5)
            ...
            for i in range(start_check, end_check - 1):
                ...
                # Pattern 1: Closing structure followed by opening structure
                ...
                # Pattern 3: Number/boolean followed by key or value
                elif (re.search(r'[0-9]|true|false|null\s*$', current_line) and
                      not current_line.endswith(',') and
                      (next_line.startswith('"') or '{' in next_line or '[' in next_line)):
                    needs_comma = True
                
                if needs_comma:
                    print(f"[JSON REPAIR] Adding comma after line {i}: {current_line[:50]}...")
                    lines[i] = lines[i].rstrip() + ','
            
            json_str = '\n'.join(lines)
            
            # Method 3: Regex-based fixes for common patterns
            # Fix missing commas after values
            json_str = re.sub(r'(["\d\}\]](\s*))([\n\r\s]*)(["\'\{\[])', r'\1,\3\4', json_str)
