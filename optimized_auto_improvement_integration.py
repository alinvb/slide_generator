...
class OptimizedAutoImprovementIntegrator:
    ...
    def auto_improve_json_if_enabled_optimized(self, json_data: Dict[str, Any], json_type: str) -> Dict[str, Any]:
        ...
        🚨 BULLETPROOF PROTECTION: Skips improvement for bulletproof-generated data
        """
        if not st.session_state.get(f"{self.session_key_prefix}enabled", False):
            return json_data
        
        # 🚨 CRITICAL PROTECTION: Check if data was generated by bulletproof system
        if isinstance(json_data, dict) and json_data.get("_bulletproof_generated", False):
            print(f"🛡️ [AUTO-IMPROVE] PROTECTED: Skipping improvement for bulletproof-generated {json_type}")
            print(f"🛡️ [AUTO-IMPROVE] Data sources: {json_data.get('_data_sources', 'unknown')}")
            st.info(f"🛡️ **Protected Data**: {json_type} generated by bulletproof system with real research data - no improvement needed")
            
            # Remove bulletproof markers before returning (clean data)
        ...
        try:
            ...
            print(f"🔧 [AUTO-IMPROVE] Processing non-bulletproof {json_type} - applying improvements...")
