...
- Ensure revenue growth is consistent with industry benchmarks and company maturity"""
    ...
    def _process_regional_conglomerates(self, merged_data: Dict):
        """Process regional conglomerates based on company geography and industry from conversation"""
        print("🌏 [REGIONAL-PROCESSING] Processing regional conglomerates from conversation...")
        ...
        for buyer in regional_strategic_buyers:
            if isinstance(buyer, str) and buyer.strip():
                regional_buyers.append({
                    "buyer_name": buyer,
                    "description": f"Regional conglomerate active in {industry}",
                    "strategic_rationale": "Regional expansion and market consolidation based on conversation",
                    "key_synergies": "Market access and operational synergies", 
                    "fit": "High (8-9/10)",
                    "financial_capacity": "Very High"
                })
        
        # Also check general strategic buyers for regional keywords
        strategic_buyers = merged_data.get('strategic_buyers_mentioned', [])
        for buyer in strategic_buyers:
            if isinstance(buyer, str) and any(region in buyer.lower() for region in ['asia', 'asian', 'southeast', 'regional', 'china', 'singapore', 'malaysia', 'thailand', 'pacific']):
                # Avoid duplicates
                if not any(existing['buyer_name'] == buyer for existing in regional_buyers):
                    regional_buyers.append({
                        "buyer_name": buyer,
                        "description": f"Regional player in {industry}",
                        "strategic_rationale": "Geographic expansion and market presence",
                        "key_synergies": "Regional market access and consolidation", 
                        "fit": "High (8-9/10)",
                        "financial_capacity": "Very High"
                    })
        
        # Adapt regional scope based on geography mentioned in conversation
        regional_scope = "SEA"  # Default
        if geography_mentioned:
            ...
            if any(region in geography_text for region in ['north america', 'usa', 'us', 'america']):
                regional_scope = "North America"
            elif any(region in geography_text for region in ['europe', 'european', 'eu', 'uk']):
                regional_scope = "Europe"
            elif any(region in geography_text for region in ['asia', 'asian', 'china', 'japan', 'southeast']):
                regional_scope = "Asia-Pacific"
        
        # If we found regional players in conversation, use them
        if regional_buyers:
            # Update the field name to be more generic based on regional scope
            if regional_scope == "SEA" or regional_scope == "Asia-Pacific":
                merged_data['sea_conglomerates'] = regional_buyers
            else:
                # For non-SEA regions, still use the same field but with region-appropriate data
                merged_data['sea_conglomerates'] = regional_buyers
            
            print(f"✅ [REGIONAL-PROCESSING] Extracted {len(regional_buyers)} regional conglomerates from conversation for {regional_scope}")
        else:
    ...
    def _process_margin_cost_resilience(self, merged_data: Dict):
        ...
        # Create margin cost data structure if we found relevant information
        if margin_insights:
            margin_data = {
                "banker_view": "Conversation analysis indicates margin pressures and cost management considerations",
